...
def generate_random_operations(
    csv_path, num_operations, max_table_index
) -> List[Dict[str, Any]]:
    """
    ...
    for _ in range(num_operations):
        ...
        if action == "edit":
            ...
            operation["details"] = {"index": row, "amount": rd.randint(1, 3)}

        elif action == "add_col":
            operation["details"] = {"index": col, "amount": rd.randint(1, 2)}

        elif action == "merge":
            is_row = rd.choice([True, False])
            index = row if is_row else col
            operation["details"] = {"index": index, "isRow": is_row}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate random JSON operations for CSV editing"
    )
    parser.add_argument(
        "--csv_path",
        "-p",
    ...
    parser.add_argument(
        ...
        "-o",
        type=str,
        default="./test/test_inst/",
        help="Output JSON file path",
    )
    parser.add_argument(
        "--num", "-n", type=int, default=50, help="Number of JSONs to generate"
    )
    parser.add_argument(
        "--max-table", "-t", type=int, default=-1, help="Maximum table index"
    )
